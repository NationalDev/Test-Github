...
function associateMessagesToRecords(messages)
{
    if(messages){
        ...
        while(i < len)
        {
            ...
            var stringResult = stringResultArr[-1];
            logDebug("Content: " + content);
            logDebug("Record ID #:>" + altIdMatch[1]);

            logDebug("First altId:"+altId);
            ...
            if (aa.cap.getCapID(altId.trim()).getSuccess())
            {
                ...
                var updateWorkflowResult = updateWorkflow(thisCapId, content, reviewTask);
...
function parseTaskFromContent(messageBody)
{
    ...
    var taskResult = taskFormat.exec(messageBody);
    return taskResult;
